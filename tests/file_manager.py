...
order_keys = ["Orders", "Customer_name", "Address", "Tel", "Courier", "order_status", "Cart", "OrderID"]
...
def download(database):
    ...
    if selection == "1":
        csv_save(products_values, load_dict(select_table_query(products_values[0])))
    elif selection == "2":
        csv_save(courier_keys, load_dict(select_table_query(courier_keys[0])))
    elif selection == "3":
        csv_save(CUSTOMER_KEYS, load_dict(select_table_query(CUSTOMER_KEYS[0])))
    elif selection == "4":
...
def sort_by(keys):
    ...
    try:
        index = int(input("\nFilter orders by: ")) -1
        test = load_dict((f"SELECT * FROM Orders WHERE order_status = '{keys[index]}' ORDER BY order_status DESC"))
...
def update_entry(database):
    try:
        ...
        cursor.execute(f"UPDATE {database[0]} SET {database[key]} = '{update}' WHERE {database[-1]} = {update_select}")
        connection.commit()
        refresh()
...
def customer_info():
    ...
    tel = input("Enter Tel: ")
    cursor.execute(f"INSERT INTO Customers (Customer, Address, Tel) VALUES (\'{name}\', \'{address}\', '{tel}')")
...
def add_order():
    ...
    sql = f"INSERT INTO Orders (Customer_name, Address, Tel, couriers, order_status, Cart) VALUES (\'{prod_inputs[0]}\', \'{prod_inputs[1]}\', \'{prod_inputs[2]}\', \'{prod_inputs[4]}\', 'PREPARING', \'{prod_inputs[3]}\')" # Runs SQL query
    refresh()
    cursor.execute(sql)
    connection.commit()
...
def update_order(order_keys):
    ...
    #If user selects 4, 5 or 6 Run the selected update functions.
        ...
        if key_update == 5:
            ...
            update_with = input("Update with?: ").title()  #Updates the order at chosen key 
            cursor.execute(f"UPDATE Orders SET {order_keys[key_update]} = %s WHERE OrderID = %s", (update_with, order_select))
            connection.commit()
    ...
    try:
       while True:
        ...
        if order_select == "":
            ...
            test = load_dict(f"SELECT * FROM Orders WHERE OrderID = {int(order_select)}")
            print("\n --Selected Order--\n")
            print_panda(test)
    except:invalid()


def update_status(order_select):                #Updates just the orders status which is called in update_order func
...
def update_Ocourier(order_select):              #Updates just the orders Courier which is called in update_order func
